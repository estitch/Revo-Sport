	...
	protected void processRequest(HttpServletRequest request,
		...
		if (request.getParameter("action").equals("create")) {
			...
			q.declareParameters("String nombreParam");
				...
				List<Cliente> clientes = (List<Cliente>)q.execute(cod);
				...
				rd.forward(request, response);
					...
					if(es==true){
						...
						List<Cliente> cliente1 = ClienteDatos.verCliente(dni);
						request.setAttribute("clientes", cliente1);
						RequestDispatcher rd = getServletContext().getRequestDispatcher("/actualizarCliente.jsp");
						rd.forward(request, response);
					}
					else{
