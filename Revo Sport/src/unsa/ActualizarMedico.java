	...
	protected void processRequest(HttpServletRequest request,
		...
		if (request.getParameter("action").equals("create")) {
			...
			q.declareParameters("String nombreParam");
				...
				List<Medico> medicos = (List<Medico>)q.execute(cod);
				...
				rd.forward(request, response);
					...
					if(es==true){
						...
						List<Medico> medicos1 = MedicoDatos.verMedico(dni);
						request.setAttribute("medicos", medicos1);
						RequestDispatcher rd = getServletContext().getRequestDispatcher("/actualizarMedico.jsp");
						rd.forward(request, response);
					}
					else{
