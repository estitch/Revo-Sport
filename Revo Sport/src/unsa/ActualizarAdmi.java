...
public class ActualizarAdmi extends HttpServlet {
...
//			Query q = pm.newQuery(Alumno.class,"name='miguel'");
			...
			q.declareParameters("String nombreParam");
				...
				List<Admi> admis = (List<Admi>)q.execute(cod);
				...
				if(es==true){
					...
					List<Admi> admi = AdmiDatos.verAdmi(dni);
					request.setAttribute("admis", admi);
					RequestDispatcher rd = getServletContext().getRequestDispatcher("/actualizarAdmi.jsp");
					rd.forward(request, response);
				}
				else{
