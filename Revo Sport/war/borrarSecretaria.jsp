...
<!-- video : fondo de pantalla-->
           ...
           <center> <div  class="caja">
            ...
            <table id="t01" class= "table">
                ...
                <% for(int i = 0; i < secretarias.size(); i++) {
                    Secretaria secretaria = (Secretaria)secretarias.get(i);%>
                <tr>
                    <td><%= secretaria.getId() %></td>
                    <td><%= secretaria.getApellidop() %> <%= secretaria.getApellidom() %>, <%= secretaria.getNombre() %></td>
                    <td><%= secretaria.getDocumento() %></td>
                    <td><%= secretaria.getDireccionp() %></td>
                    <td><%= secretaria.getTelefono() %></td>
                    <td><%= secretaria.getEmail() %></td>
                    <td><%= sdf.format(secretaria.getFecha()) %></td>
                </tr>
            <%}%>
    ...
    <center>
        <div class="caja">
            <form class="contact_form" action="/borrarSecretaria" method="post">
                <li class="cliente">
                    <h2>BORRAR SECRETARIA</h2>
                </li>
                <div id='izquierda'>
                    <ul>

                        <li><label >Ingrese DNI: </label> 
                            <input type="text" name="dni" maxlength="8" pattern="[0-9]{8}" title="Ingrese solo numeros">
